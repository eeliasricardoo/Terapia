...
async function main() {
    ...
    `)
        ...
        // 2. Drop existing policies individually
        await prisma.$executeRawUnsafe(`DROP POLICY IF EXISTS "Public Access" ON storage.objects;`)
        await prisma.$executeRawUnsafe(`DROP POLICY IF EXISTS "Authenticated users can upload avatars" ON storage.objects;`)
        await prisma.$executeRawUnsafe(`DROP POLICY IF EXISTS "Authenticated users can update avatars" ON storage.objects;`)
        await prisma.$executeRawUnsafe(`DROP POLICY IF EXISTS "Authenticated users can select avatars" ON storage.objects;`)
