    ...
    describe('getPatientPublicEvolutions', () => {
        ...
        it('should return formatted evolutions with psychologist names', async () => {
            ...
            (prisma.evolution.findMany as jest.Mock).mockResolvedValue([
                { id: 'ev-1', patientId: 'profile-1', psychologistId: 'psych-1', publicSummary: 'Progresso', mood: 'Bem', date: now },
                { id: 'ev-2', patientId: 'profile-1', psychologistId: 'psych-2', publicSummary: null, mood: 'Neutro', date: now },
            ]);
            ...
            expect(result[0]).toEqual(expect.objectContaining({ id: 'ev-1', psychologistName: 'Dra. Ana', mood: 'Bem' }))
            expect(result[1]).toEqual(expect.objectContaining({ id: 'ev-2', psychologistName: 'Dr. Carlos', publicSummary: null }))
        })
        ...
        it('should use fallback name when psychologist is not found', async () => {
            ...
            (prisma.profile.findUnique as jest.Mock).mockResolvedValue({ id: 'profile-1' });
            (prisma.evolution.findMany as jest.Mock).mockResolvedValue([
                { id: 'ev-1', psychologistId: 'unknown', publicSummary: 'S', mood: 'Bem', date: new Date() },
            ]);
            (prisma.psychologistProfile.findMany as jest.Mock).mockResolvedValue([])
